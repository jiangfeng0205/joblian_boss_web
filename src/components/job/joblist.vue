    ...
    <div id="main">
        ...
        <div class="main_fixed_top">
            ...
            <div class="messge_bar clear" style="background: #ccc;">
                <div class="message_box" style="width:100%;">
                    <div v-for="(nav,index) in messageNav">
                        <span style="width:50%;height:1.28rem;display: flex;align-items: center;justify-content: center;"
                           :class="{on:nav.isOn}"
        ...
        <div class="job_content" id="jobcontent" ref="wrapper" style="padding-top: 12%;">
            <ul class="page-infinite-list job_lists" v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="50">
                <li v-for="(job,index) in jobs" :key="index" @click="hrefs(job.id,job.fromId,job.userType,job.toId,job.company_parent_uid,job.company_uid,job.companyid,job.m_id,job.parent_uid,job.fromName,job.toName,job.belongid,job.provinceName,'joblist')">
                    ...
                    <a v-if="job" >
                        ...
                        <p class="clear title">
                            ...
                            <!-- // <span class="megNumber"  v-if="job.noread > 0">{{job.noread}}</span> -->
                        ...
                        <p v-if = "job.userType == 3">顾问</p>
                        <p v-if = "job.userType == 2">HR</p>
                        <p v-if = "job.userType == 1">候选人</p>
                    </a>
                    <!-- </router-link> -->
                </li>
            </ul>
    ...
    export default {
        ...
        sockets: {
            ...
            initToUserOnline(value) {
                //与socket.io连接后回调
                console.log("connectedjoblist---",value);
            },
        ...
        data () {
            return {
                ...
                messageNav:[
                    {title:'推荐职位',isOn:true,num:0,url:'joblist'},
                    {title:'我的职位',isOn:false,num:0,url:'joblistMyself'}
                ],
                slideIndex:"",
                isShowSlide:false,
                slideTemp:[],
        ...
        methods: {
            ...
            go(v,k){
                if (k == 1) {
                    this.$router.push({path:"/joblistMyself"});
                    // window.location.href = "/joblistMyself";
                }
            },
            ...
            changeColor(nav,index){
                ...
                if (this.slideIndex == 0) {
                    console.log('navselcted--', nav.isSelected)
                }
                console.log('slideIndex--',this.slideIndex);
                console.log('slideTemp--',this.slideTemp);
            },
            willscroll(){
                //2.1 使用定时器，防止频繁滚动
