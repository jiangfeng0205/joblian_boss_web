<template>
    <div class="login-bg">
        <colunmLabs ref="judge" :loginBox="loginBox" :classType="classType" :dericteUrl="dericteUrl"  :tableTitle="tableTitle" :tabData="tabData1" :nowIndex="nowIndex" @hide="hide" @indexData="indexData" @changeList="changeList" @btnSure="btnSure" @getCityOrhy="getCityOrhy" v-show="showHide"></colunmLabs>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        data () {
            return {
                loginBox : true,
                dericteUrl : '/company/companyjobadd',
                tableTitle : '请选择地域',
                classType  : 'city',
        ...
        mounted(){
            let _self=this;
            _self.getCityOrhy('edwinbj_city_class', 0);
            var jobId = this.$route.query.jobId;
            if (jobId > 0) {
                this.dericteUrl = '/company/companyJobDetail/'+jobId;
            }
