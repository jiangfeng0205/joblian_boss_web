<template>
    <div class="login-bg">
      <div id="login">
        <form action="">
          ...
          <ul class="info-login">
            ...
            <li><input @click.prevent="login" type="submit" value="进入"></li>
          </ul>
        </form>
      </div>
      <!--底部-->
      <div class="login-footer flex_parent">
        ...
        <router-link to="/register1" class="flex_child"><span>注册</span></router-link>
      </div>
      <!-- selected role -->
      <mt-actionsheet
        :actions="actions"
    ...
    export default {
        ...
        methods: {
          ...
          selectAdminRole(){
              ...
              Store.doSave('userName', this.user.account);
          ...
          login(){
              ...
              axios({
                  ...
                  if (result.code == 200) {
                      _self.userOnlineLoginout(result.data.uid,result.data.usertype[0]);
                      Store.save(result.data.uid);
                      // Store.saveLoginJump('/company');
                      Store.saveUserType(result.data.usertype[0]);
                      Store.doSave('userName', _self.user.account);
